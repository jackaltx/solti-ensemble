...
get_ispconfig_domains() {
    echo "Extracting domains from ISPConfig database..."
    ...
    if [[ ${#domains[@]} -eq 0 ]]; then
        echo "Database query failed, checking for existing BOM audit..."
        local bom_file
        for bom_file in "/opt/audit/ispconfig-bom-audit/ispconfig-bom-config.json" \
            ...
            if [[ -f "$bom_file" ]] && command -v jq &> /dev/null; then
                echo "Found BOM audit file: $bom_file"
                while IFS= read -r domain; do
                    [[ -n "$domain" ]] && domains+=("$domain")
    ...
    if [[ ${#domains[@]} -eq 0 ]]; then
        echo "No domains found via database or BOM audit, scanning configuration files..."
...
get_file_domains() {
    ...
    echo "Reading domains from file: $DOMAINS_FILE"
    grep -v '^#' "$DOMAINS_FILE" | grep -v '^$' | sort -u
}
